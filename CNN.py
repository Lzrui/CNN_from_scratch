...
class Conv3x3:
    ...
    def __init__(self, num_filters,activate = None):
    ...
    def forward(self, input):
        ...
        if self.activate == 'ReLU':
            self.outout_r = np.maximum(output,0)
            return self.outout_r
        return output
    ...
    def backprop(self,d_L_d_out,learn_rate):
        ...
        # 如果为ReLU激活函数，则进行梯度计算
        if self.activate == 'ReLU':
            self.outout_r[self.outout_r>0] = 1
            d_L_d_out = d_L_d_out * self.outout_r

        # 初始化一组为0的梯度矩阵，尺寸为3*3*8
        d_L_d_filters = np.zeros(self.filters.shape)

        # im_region,一个个3*3的矩阵
        for im_region,i,j in self.iterate_regions(self.last_input):
            for f in range(self.num_filters):
                #对不同的filter进行梯度运算,即点乘im_region
...
class MaxPool2:
    ...
    def backprop(self, d_L_d_out):
        ...
        # 每一个 im_region 都是一个2*2*8的8层小矩阵
        # 修改max的部分，首先查找max
        for im_region,i,j in self.iterate_regions(self.last_input):
            h,w,f = im_region.shape
            # 获取 im_region里最大值(其对应索引绝大部分不止一处（总数为4处）)
            amax = np.amax(im_region,axis=(0,1))

            #遍历整个 im_region, 对于传递下去的像素点，修改 gradient 为 loss 对 output 的 gradient
            for i2 in range(h):
                for j2 in range(w):
                    for f2 in range(f):
...
conv = Conv3x3(8,activate="ReLU")                                    # 28x28x1 -> 26x26x8
